...
public class MyCalendarView extends LinearLayout implements OnClickListener {
	...
	private static MyCalendarView calendarView;
	...
	public void goToday(int mjumpMonth,int mjumpYear) {
		Log.i(TAG, "goToday: "+mjumpMonth+":"+mjumpYear);
		addGridView(); // 添加一个gridView
		jumpMonth++; // 下一个月
		adapter = new CalendarAdapter(mContext, this.getResources(), mjumpMonth,
				mjumpYear, year_c, month_c, day_c);
		gridView.setAdapter(adapter);
		addTextToTopTextView(currentMonth); // 移动到下一月后，将当月显示在头标题中
		flipper.addView(gridView, 0);
		flipper.setInAnimation(AnimationUtils.loadAnimation(mContext,
